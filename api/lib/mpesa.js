...
const getPassword = () => {
  const now = new Date();
  const eatTime = new Date(now.getTime() + (3 * 60 * 60 * 1000)); // EAT is UTC+3
  const timestamp = eatTime.toISOString().slice(0, 19).replace(/[-T:]/g, '');
  const password = Buffer.from(`${MPESA_CONFIG.shortCode}${MPESA_CONFIG.passkey}${timestamp}`).toString('base64');
  return { password, timestamp };
};

const initiateSTKPush = async (phone, amount, invoiceId, description = 'Nuru Foundation Payment') => {
  try {
...
const simulatePayment = (invoiceId, amount) => {
  return {
    success: true,
