...
const router = {
  ...
  async handle(req, res) {
    ...
    console.log(`[Admin Router] Handling ${req.method} ${path}`);
    console.log(`[Admin Router] Available routes:`, Object.keys(this.handlers));
    
    const handler = this.handlers[`${req.method}:${path}`];

    if (handler) {
      console.log(`[Admin Router] Handler found for ${req.method}:${path}`);
      try {
      ...
      console.log(`[Admin Router] No handler found for ${req.method}:${path}`);
